...
import { useAxios } from '@admin/composables/axios';
import { useAlert } from 'balm-ui';

import SingleMap from '@/components/Map.vue';

import campusList from '@/assets/js/campus';
import oldMarkers from '@/assets/js/oldMarkers';
      ...
      onAdd: async () => {
        ...
        try {
          if (!uiState.dialog.modifyMarker.withDeleteAction) {
            await axios.put('/marker', submitMarker);
          } else {
    ...
    if (!bn.has(x)) {
      ...
      if (
        ...
        if (mo.openTime !== mn.openTime) {
          diff = diff.concat(`开放时间: ${mo.openTime} => ${mn.openTime}\n`);
        }
        if (mo.extraInfo !== mn.extraInfo) {
          diff = diff.concat(`附加信息: ${mo.extraInfo} => ${mn.extraInfo}\n`);
        }
        diffMarkers.push({
          color: 'yellow',
          ...bn.get(x),
...
<template>
  ...
  <ui-drawer nav-id="demo-menu" type="modal">
    ...
    <ui-drawer-content>
      <ui-nav>
        <template v-for="campus of campusList" :key="campus.id">
          <ui-nav-item
  ...
  >
    ...
    <ui-dialog-content>
      ...
      <div style="display: flex; justify-content: center; margin-top: 1rem">
        <ui-button raised @click.prevent="uiState.dialog.settings.open = false">
          确认
        </ui-button>
      </div>
    </ui-dialog-content>
      ...
      <ui-form type="|" item-margin-bottom="16" action-align="center">
        <template #default="{ actionClass }">
          ...
          <ui-form-field class="required">
            ...
            <ui-textfield
              fullwidth
              v-model="uiState.dialog.modifyMarker.marker.englishName"
            ></ui-textfield>
          </ui-form-field>
          <ui-form-field class="required">
            <label>坐标</label>
            <ui-textfield fullwidth disabled>{{
              uiState.dialog.modifyMarker.marker.position.map(x => parseInt(x))
            }}</ui-textfield>
          </ui-form-field>
          <ui-form-field>
            <label>类型</label>
          ...
          <ui-form-field class="required">
            ...
            <ui-textfield
              fullwidth
              v-model="uiState.dialog.modifyMarker.marker.openTime"
            ></ui-textfield>
          </ui-form-field>
          <ui-form-field>
            <label>附加信息</label>
            <ui-textfield
              ...
              input-type="textarea"
              rows="5"
              v-model="uiState.dialog.modifyMarker.marker.extraInfo"
            ></ui-textfield>
          </ui-form-field>
          <ui-form-field v-show="uiState.dialog.modifyMarker.marker.tips">
            <label>其它</label>
            <ui-textfield
              fullwidth
              disabled
          ...
          <ui-form-field
            :class="actionClass"
            v-show="!uiState.dialog.modifyMarker.loading && !mapState.onlyDiff"
          >
            <ui-button raised @click.prevent="uiState.dialog.modifyMarker.onAdd"
              >提交</ui-button
            >
            <ui-button
              outlined
              @click.prevent="uiState.dialog.modifyMarker.onCancel"
              >取消</ui-button
            >
            <ui-button
              raised
              @click.prevent="uiState.dialog.modifyMarker.onDelete"
              v-show="uiState.dialog.modifyMarker.withDeleteAction"
              >删除此标记</ui-button
            >
          </ui-form-field>
          <ui-form-field
  ...
  <div id="content-box">
    ...
    <template v-for="campus in campusList" :key="campus.id">
      <single-map
